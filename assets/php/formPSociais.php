...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if($isDev){
        $uploaddir = 'C:\Users\x900l\Desktop\MECTRONIC\mectronic_nuxt\public\media\psociais\\';
    }else{
        $uploaddir = dirname(__DIR__,1)."/media/psociais/";
    }
    ...
    if ($json->emEdicao == false) {
        ...
        $sql = " INSERT INTO psociais (texto, texto_en, texto_es, arquivo) VALUES ('$texto','$textoEN', '$textoES', '$ARRAY_NOME_ARQ[0]')";
        $msg = "Registro inserido com sucesso";
    } else {
        $id = $json->id;

        // FAZ CÓPIA DA imagem ORIGINAL
        $sqlCopia = "SELECT * FROM psociais WHERE id = $id";
        ...
        $sql = "UPDATE psociais SET texto='$texto', texto_en='$textoEN', texto_es='$textoES' $ARQUIVO  WHERE id = $id ";
