...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_GET['q'])) {
        ...
        if (isset($_GET['tag'])) {
            $t = $_GET['tag'];
            $whereQuery = "LOWER(tags) LIKE LOWER('%$t%')";
        } else {
            $whereQuery = "LOWER(titulo) LIKE LOWER('%$q%') OR LOWER(titulo_en) LIKE LOWER('%$q%') OR LOWER(titulo_es) LIKE LOWER('%$q%') OR LOWER(descricao) LIKE LOWER('%$q%') ";
        }
    }
    ...
    if ($tipoSql == 1) {
        $sql = "SELECT *, DATE_FORMAT(criado_em,'%d de %M de %Y') AS data_br FROM blog_posts 
         $xLimit";
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        if ($json->emEdicao == false) {
            ...
            $sql = " INSERT INTO blog_posts (titulo, titulo_en, titulo_es, criado_em, descricao, descricao_en, descricao_es, categoria_id, blog_autor_id, tags) 
        VALUES ('$titulo', '$tituloEN', '$tituloES', '$criado_em', '$descricao', '$descricaoEN', '$descricaoES', $categoria_id, $blog_autor_id, '$tags')";
