...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if (isset($_GET['q'])) {
        ...
        if ($_GET['q'] == 'blog_home') {
            ...
            $tipoSql = 0;
            $q =$_GET['q'];
            $whereQuery = "WHERE LOWER(bp.titulo) LIKE LOWER('%$q%') OR LOWER(bp.titulo_en) LIKE LOWER('%$q%') OR LOWER(bp.titulo_es) LIKE LOWER('%$q%')";
...
if ($_SERVER['REQUEST_METHOD'] == 'DELETE') {
    ...
    $sqlBlogCopy = "SELECT * FROM blog_post WHERE id = $id";
    $result = mysqli_query($conn, $sqlBlogCopy) or die("Error in Selecting " . mysqli_error($connection));
    print_r($result);
    $R['codigo'] = 1;
    $R['msg'] = "Erro atualizando registro: " . $conn->error;
    $R['dados'] = $result;
