...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($tipoSql == 1) {
        ...
        $sql = "select * from solucao_linhas WHERE ativo = 1 ORDER BY ordem ASC $xLimit";

    } elseif ($tipoSql == 2) {
        $sql = "SELECT s.*, sl.link FROM solucoes AS s LEFT JOIN solucao_linhas AS sl ON s.linha_id = sl.id WHERE s.linha_id = $linhaSolucaoId AND ativo = 1 ORDER BY ordem ASC $xLimit";
    } elseif ($tipoSql == 3) {
        ...
        $sql = "SELECT s.*, sl.link FROM solucoes AS s LEFT JOIN solucao_linhas AS sl ON s.linha_id = sl.id WHERE $whereQuery $xLimit ORDER BY ordem ASC";
    }
