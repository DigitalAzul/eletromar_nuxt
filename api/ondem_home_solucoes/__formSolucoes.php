...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    if ($tipoSql == 1) {
        ...
        $sql = "select * from solucao_linhas $xLimit";

    } elseif ($tipoSql == 2) {
        $sql = "SELECT s.*, sl.link FROM solucoes AS s LEFT JOIN solucao_linhas AS sl ON s.linha_id = sl.id WHERE s.linha_id = $linhaSolucaoId  $xLimit";
    } elseif ($tipoSql == 3) {
        $sql = "SELECT s.*, sl.link FROM solucoes AS s LEFT JOIN solucao_linhas AS sl ON s.linha_id = sl.id WHERE s.id = $SolucaoId";
    } else {
        $sql = "SELECT s.*, sl.link FROM solucoes AS s LEFT JOIN solucao_linhas AS sl ON s.linha_id = sl.id WHERE $whereQuery $xLimit";
    }
