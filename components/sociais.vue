<template>
  <div envelope="Sociais">
    <div class="drawLineContSoc">
      <div
        class="drawLineInnerContSoc"
        id="drawLineInnerContIDSoc"
        style="
            ...
            <div class="min-h-[140px] w-full pt-2">
              ...
              >
                <a
                  ...
                  target="_blank"
                  :href="pathPsociais + psocial[0].arquivo"
                >
...
onMounted(() => {
  ...
  tlContTitCont.from(".H1Social", {
    duration: 0.6,
    xPercent: 100,
    ease: "power4",
    stagger: 0.1,
  });
  tlContTitCont.from(".H2Social", {
    duration: 0.6,
    xPercent: -100,
    ease: "power4",
    stagger: 0.1,
  ...
  function setTranslate(pg) {
    let yInicial = pg;
    let txSocialIDspacer = document.getElementById("txSocialID");
    if (yInicial > ultimaPosicao.value) {
      ...
      if (atualY.value < 100) {
        ...
        drawLineInnerContIDSoc.style.transform = `translate3d(0, ${atualY.value}%, 0)`;
        if (pg > 0.2) txSocialIDspacer.style.letterSpacing = `${pg}em`;
      } else {
        atualY.value = 100;
      }
    } else {
...
let itempSocial = ref([]);
const textopSocialLang = ref({});

function blocksToArray() {
  doisParagrafos.value = [];
  itempSocial.value = [];

  psocial.value.map((g, index) => {
    if (lang.value == "pt") {
      textopSocialLang.value = JSON.parse(g.texto);
    } else if (lang.value == "en") {
      textopSocialLang.value = JSON.parse(g.texto_en);
    } else {
      textopSocialLang.value = JSON.parse(g.texto_es);
    }

    textopSocialLang.value.map((b, index) => {
      if (index < 2) {
        doisParagrafos.value.push(b);
      } else {
        itempSocial.value.push(b);
      }
    });
  });
}
async function getpSocial() {
  let R = await $fetch("/api/formPSociais.php", {
    method: "GET",
  }).then((R) => {
    let r = JSON.parse(R);
    if (r.codigo == 0) {
      psocial.value = r.dados;
      blocksToArray();
    } else {
    }
  });
