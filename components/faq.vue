<template>
  <div envelope="Politica">
    <div class="drawLineContFaq">
      <div
        class="drawLineInnerContFaq"
        id="drawLineInnerContIDFaq"
        style="
    ...
    >
      <div
        id="txfaqID"
        class="container relative mx-auto text-center text-[2.5rem] font-normal tracking-[.3em] text-white md:top-20 md:text-6xl"
      >
        FAQ
      </div>
    </div>
...
onMounted(() => {
  ...
  tlContTitCont.from(".H1faq", {
    duration: 0.6,
    xPercent: 100,
    ease: "power4",
    stagger: 0.1,
  });
  tlContTitCont.from(".H2faq", {
    duration: 0.6,
    xPercent: -100,
    ease: "power4",
    stagger: 0.1,
  ...
  function setTranslate(pg) {
    let yInicial = pg;
    let txfaqIDspacer = document.getElementById("txfaqID");
    if (yInicial > ultimaPosicao.value) {
      ...
      if (atualY.value < 100) {
        ...
        drawLineInnerContIDFaq.style.transform = `translate3d(0, ${atualY.value}%, 0)`;
        if (pg > 0.2) txfaqIDspacer.style.letterSpacing = `${pg}em`;
      } else {
        atualY.value = 100;
      }
    } else {
...
function blocksToArray() {
  ...
  faq.value.map((g, index) => {
    if (lang.value == "pt") {
      textoFaqLang.value = JSON.parse(g.texto);
    } else if (lang.value == "en") {
      textoFaqLang.value = JSON.parse(g.texto_en);
    } else {
      textoFaqLang.value = JSON.parse(g.texto_es);
    }

    textoFaqLang.value.map((b, index) => {
      if (index < 2) {
        doisParagrafos.value.push(b);
      } else {
        itemFaq.value.push(b);
      }
    });
  });
}
